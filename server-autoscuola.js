...
app.get('/getGuideOggi', function(req, res) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    const date = new Date()
    pool.query("SELECT * FROM guida JOIN cliente ON guida.cliente_g = cliente.cod_fis WHERE guida.data_guida = $1 ", 
...
app.get('/search_user', function(req, res) {
    ...
    " $3::DATE is null) AND (cod_fis=$4::varchar OR $4::varchar IS NULL)", 
    [request["nome"], request["cognome"], request["dn"], request["cf"]], function(err, result) {
        if (err) {
            console.error(err)
        }
        res.json(result);
...
app.post('/user_add', function(req, res) {
    ...
    if (req.body.nome1 == null || req.body.cognome1 == null || req.body.cf1.length != 16 ||
        ...
        " VALUES ($1::varchar, $2::varchar, $3::varchar, $4::date, $5::date, $6::varchar, $7::varchar, $8::varchar)",
         [request["cf"], request["nome"], request["cognome"], request["dn"], request["di"], request["ln"], request["telefono"], request["email"]], function(err, result) {
...
app.post('/guide', function(req, res) {
    ...
    if (req.body.nome == "" || req.body.cognome == "" || req.body.patente == "" || req.body.dt == "" || req.body.istruttore == "") {
        ...
        " cliente.cognome=$2 AND (cliente.data_nascita=$3::date OR $3::varchar IS NULL ) AND r1.patente_1=$4::varchar",
         [request["nome"], request["cognome"], request["dn"], request["patente"]], function(err, result) {
            if (err) {
                console.log(err.message)
                res.status(500).send(err.message)
            } else {
                ...
                console.log(cf)
                pool.query("INSERT INTO guida VALUES ($1::date, $2::time, $3::float, $4::float, $5::varchar, $6::varchar, $7::varchar)",
                [new Date(request["date"]), request["time"], costo_mezzora * request["durata"], request["durata"] * 30, cf, request["patente"], request["istruttore"]], function(err) {
                    if (err) {
                        console.log(err)
                        res.status(500).send(err.message)
                    } else
...
app.get('/g_search', function(req, res) {
    ...
    " JOIN cliente as c ON c.cod_fis = g.cliente_g WHERE (c.nome= $1  AND c.cognome = $2 ) OR g.data_guida = $3", 
    [request["nome"], request["cognome"], request["data"]], function(err, result) {
        if (err) {
            console.log(err)
            res.status(500).send(err.message)
        } else
...
app.get('/acconti', function(req, res) {
    ...
    " WHERE (cliente.nome= $1 OR $1 IS NULL) AND (cliente.cognome = $2 OR $2='')  AND ( cliente.cod_fis= $3 OR $3='')",
    [request["nome"], request["cognome"], request["cf"]], function(err, result) {
        if (err) {
            console.log(err.message)
            res.status(500).send(err.message)
        } else {
...
app.post("/acc_mod", function(req, res) {
    ...
    console.log(request)
    pool.query("UPDATE r1  SET acconto1 = $1::float, acconto2= $2::float WHERE patente_1 = $3::varchar AND cliente_1 = $4::varchar",
     [request["acc1_new"], request["acc2_new"], request["patente"], request["cod_fis"]], function(err, result) {
        if (err) {
            console.log(err.message)
            res.status(500).send(err.message)
        } else {
...
app.post('/esami', function(req, res) {
    ...
    if (req.body.codfis == "" || req.body.patente == "" || req.body.tipo == "" || req.body.dt == "") {
        ...
        pool.query("INSERT INTO esame VALUES (DEFAULT, $1::varchar, $2::date, $3::varchar, $4::varchar) RETURNING id",
         [request["tipo"], new Date(request["date"]), request["patente"], request["codfis"]], function(err, result) {
            if (err) {
                console.log(err)
                res.status(500).send(err.message)
            } else{
